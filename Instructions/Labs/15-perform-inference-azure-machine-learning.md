...
lab:
  ...
  description: この演習では、Azure Machine Learning モデルをデプロイし、これを使用して、azureai 拡張機能により Azure Database for PostgreSQL フレキシブル サーバーから推論を実行する方法を学びました。
  duration: 60 minutes
  level: 500
  islab: true
  primarytopics:
...
5. **[モデルのアップロード]** メニューで、モデルの種類を **[MLflow]** に設定します。 次に、**[参照]** を選択して **mlflow-model** フォルダーに移動し、アセットをアップロードします。 その後、**[次へ]** ボタンを選択して次に進みます。
    ...
    > 次に、ID ベースの認証を使うようにワークスペース データストアを切り替えます。 Azure ML スタジオで、**[データ]** > **[データストア]** > **[workspaceblobstore]** に移動し、**[認証の更新]** を選んで、**[データ アクセス用にデータストアに資格情報を保存する]** トグルを **[オフ]** にします。 変更を 保存 します。 アクセス許可が反映されるまで数分待ってから、モデルの登録をもう一度試してみてください。
...
2. 次のコマンドを実行して、`rentals` データベースに接続します。`<server-name>` を PostgreSQL フレキシブル サーバーの名前に置き換えます (Azure portal の PostgreSQL リソースの **Overview** ページに記載されています)。
...
4. この演習の残りの部分は Cloud Shell で作業を続けるので、ペインの右上にある **[最大化]** ボタンを選択して、ブラウザー画面内にペインを広げると便利な場合があります。
    ...
    ![[最大化] ボタンが赤い四角で強調表示されている Azure Cloud Shell 画面のスクリーンショット。](media/19-azure-cloud-shell-pane.png)
