...
この演習では、psql と Visual Studio Code Postgres 拡張機能をダウンロードしてインストールします。 お使いのマシンに Visual Studio Code とその Postgres 拡張情報が既にインストールされている場合は、「Azure Database for PostrgreSQL フレキシブル サーバーに接続する」に進んでかまいません。
...
psql をローカルにインストールするか、Visual Studio Code PostgreSQL 拡張機能を使用して、Azure Database for PostgreSQL サーバーに接続できます。

## psql を使用して PostgreSQL に接続する

コマンド ラインを使用する方がいい場合は、psql コマンド ライン ツールを使用して PostgreSQL サーバーに接続できます。 psql コマンド ライン ツールは、インストール済みの PostgreSQL に含まれています。
...
1. **psql**がインストールされていることを確認したら、コマンド プロンプトまたはターミナル ウィンドウを開き、コマンド ラインを使用して PostgreSQL サーバーに接続できます。
...
1. コマンド プロンプトで、「**`--host=<servername>.postgres.database.azure.com`**」と入力します。ここで、`<servername>` は先ほど作成した Azure Database for PostgreSQL の名前です。

    サーバー名は、Azure portal の **[概要]**、または bicep スクリプトからの出力で確認できます。
...
1. これで、サーバーに接続しデータベースを作成したので、データベースでのテーブルの作成など、使い慣れた SQL クエリを実行できます。
...
これでこの演習は完了です。 コマンド ラインと Visual Studio Code PostgreSQL 拡張機能を使用して PostgreSQL に接続する方法を学びました。 また、データベースを作成し、そのデータベースに対して SQL クエリを実行する方法についても学びました。
