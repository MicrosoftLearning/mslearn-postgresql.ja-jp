...
1. **[Review + create](レビュー + 作成)** を選択します。 設定を確認したあと、**[作成]** を選択して、Azure Database for PostgreSQL フレキシブル サーバーを作成します。 デプロイが完了したら、次の手順 **[リソースに移動]** を選択して次の手順の準備を行います。
...
1. 演習スクリプト ファイルがあるフォルダーに移動するか、[MSLearn PostgreSQL Labs](https://github.com/MicrosoftLearning/mslearn-postgresql/Allfiles/Labs/02) から **Lab2_ZooDb.sql** をダウンロードします。
1. Azure Data Studio がまだ開いていない場合は、開きます。
