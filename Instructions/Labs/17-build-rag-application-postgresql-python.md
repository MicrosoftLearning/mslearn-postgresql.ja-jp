...
lab:
  ...
  description: 次のコマンドでは、この演習で使用されるすべてのリソースを格納するリソース グループに使用する名前が割り当てられます。 対応する変数に割り当てられたリソース グループ名は rg-learn-postgresql-ai-$REGION です。$REGION は前に指定した場所です。 ただし、この部分は好みに合わせて他のリソース グループ名に変更できます。
  duration: 55 minutes
  level: 500
  islab: true
  primarytopics:
...
1. この演習では、2 つの方法のうち **1 つ**を使用して Azure OpenAI に対して認証を行います。 ご自分の環境に該当するものを選び、各ステップではそれらの手順にのみ従ってください。
    ...
    **マネージド ID** をお使いで、その設定をまだ行っていない場合は、ここで次のコマンドを実行して行います。 それ以外の場合、次のステップに進みます。
...
1. 次のコマンドを実行して、`ContosoHelpDesk` データベースに接続します。`<server-name>` は、実際の PostgreSQL フレキシブル サーバーの名前に置き換えます (Azure portal の PostgreSQL リソースの **[概要]** ページに記載されています)。
...
1. この演習の残りの部分では、Cloud Shell で作業を続けます。そのため、ペインの右上にある **[最大化]** ボタンを選択して、ブラウザー ウィンドウ内でペインを拡大すると作業しやすくなります。
    ...
    ![[最大化] ボタンが赤い四角で強調表示されている Azure Cloud Shell 画面のスクリーンショット。](./media/azure-cloud-shell-pane-contoso.png)
...
1. 両方の拡張機能を許可リストに追加するには、[PostgreSQL 拡張機能の使用方法](https://learn.microsoft.com/azure/postgresql/flexible-server/concepts-extensions#how-to-use-postgresql-extensions)に関する記事で説明されている手順に従って、サーバー パラメーター `azure.extensions` に `vector` と `azure_ai` を追加します。

1. 次の SQL コマンドを実行して、`vector` および `azure_ai` 拡張機能を有効にします。 詳細な手順については、[Azure Database for PostgreSQL で `pgvector` を有効にして使用する方法](https://learn.microsoft.com/azure/postgresql/flexible-server/how-to-use-pgvector#enable-extension)に関する記事を参照してください。
...
1. 次に、Azure OpenAI への `azure_ai` 拡張機能の接続を構成します。 Azure OpenAI リソースのエンドポイントが必要です (Azure portal の **[リソース管理]** の下の **[キーとエンドポイント]** ページにあります)。

    ![KEY 1 とエンドポイントのコピー ボタンが赤い四角で強調表示された、Azure OpenAI サービスの [キーとエンドポイント] ページのスクリーンショットが表示されます。](media/azure-openai-keys-and-endpoints.png)

    *ContosoHelpDesk* のプロンプトが表示されたら、選択した認証方法のコマンドを実行します。
...
1. 既存の行に対する埋め込みをバックフィルします。
    ...
    **psql** セッション (Cloud Shell) で次のコマンドを実行し、埋め込みを複数のバッチで計算します。 これにより、行が小さいグループで処理され、各バッチ間で一時停止されるため、"**429 要求が多すぎる**" エラーが回避されます。
    ...
    > **注:** 調整が繰り返し発生する場合は、`batch_size` を減らすか (40、30、または 20 を試します)、`pause_secs` を増やします (20 または 30 を試します)。

    108 行がすべてバックフィルされたら、次のセクションに進みます。

### 類似性クエリを使用してベクトル テーブルをテストする

すべてが機能していることを確認するために、類似性検索と単純なフィルター処理を SQL から直接実行してみましょう。

1. Azure Cloud Shell で、前と同様に *psql* を使って *ContosoHelpDesk* データベースに再び接続します。
...
返される回答は、手始めとしては悪くありませんが、クエリがさらに複雑になると網羅性が不足するおそれがあります。 この問題に対処するには、データベースから関連するパッセージを取得し、それらを回答を生成するためのコンテキストとして使用する Python RAG (検索拡張生成) アプリケーションを作成します。
...
1. PostgreSQL と Azure OpenAI の資格情報を使用して、`.env` ファイルを更新します。 選んだ認証方法と一致するテンプレートを使います。
    ...
    > **マネージド ID を使用:** `AZURE_OPENAI_API_KEY` 行を省略します。 Python アプリは、`DefaultAzureCredential` と前に割り当てたロールを使って認証を行います。
...
1. コメント "**# Azure OpenAI を呼び出し、指定されたコンテキストを使用して回答する**" を、お使いの認証方法と一致するスクリプトに置き換えます。
    ...
    > **マネージド ID を使用:** `api_key` をトークン プロバイダーに置き換えます。 また、`azure-identity` を `requirements.txt` に追加するか、`pip install azure-identity` を実行します。
    ...
    どちらのバージョンでも同じ結果が生成されます。唯一の違いは、Azure OpenAI に対する認証方法です。
