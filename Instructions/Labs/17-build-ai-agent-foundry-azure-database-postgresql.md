...
lab:
  ...
  description: この演習では、Margie's Travel 社がセマンティック検索を使用して Azure Database for PostgreSQL から物件一覧を取得するインテリジェント エージェントを作成するのを支援します。 このエージェントは Foundry Agent Service で実行され、軽量の Python Azure Functions を使用して PostgreSQL を呼び出します。PostgreSQL では、azureai および pgvector 拡張機能により、独自の AI 作業が実行されます。
  duration: 70 minutes
  level: 400
  islab: true
  primarytopics:
...
1. この演習では、2 つの方法のうち **1 つ**を使用して Azure OpenAI に対して認証を行います。 環境に適用する方を選択し、各手順で該当する指示のみに従います。
    ...
    **マネージド ID** を使用している場合は次のコマンドを実行して設定します。 それ以外の場合、次のステップに進みます。
...
1. 次のコマンドを実行して、`rentals` データベースに接続します。`<server-name>` を PostgreSQL フレキシブル サーバーの名前に置き換えます (Azure portal の PostgreSQL リソースの **Overview** ページに記載されています)。
...
ラボ リポジトリには、Azure Functions を構成する 3 つの事前構築済みファイルと、デプロイ用の事前構築済みの ZIP ファイルが含まれています。 それらを確認して役割を理解し、必要に応じて変更できるようにする必要があります。
...
| `rental-search-func.zip` | 上記の 3 つの事前構築済み ZIP ファイル (デプロイの準備完了) |
...
- PostgreSQL の `azure_openai.create_embeddings()` 関数を呼び出してクエリ テキストから埋め込みを生成し、pgvector の `<->` 演算子を使用して最も類似性の高いリストを見つけて、**ベクトル検索を実行します**。
- Foundry エージェントで使用できるように書式設定されたプロパティ ID、名前、説明、種類、価格を含む **JSON 結果を返します**。
- **関数レベルの認証 (`AuthLevel.FUNCTION`) が必要**なため、有効な関数キーを持つ呼び出し元のみがアクセスできます。

> **ヒント:** 検索の動作をカスタマイズする場合 (価格範囲やプロパティの種類でフィルター処理する場合など)、ZIP ファイルをデプロイして再作成する前に `function_app.py` を変更します。
...
1. Cloud Shell から**関数コードをデプロイ**します。
   ...
   > **注:** `401 Unauthorized` エラーが発生した場合、トークンの有効期限が切れている可能性があります。 `TOKEN=...` および `curl` コマンドをもう一度実行します。

   リモート ビルドが Python の依存関係をインストールするまで 1、2 分待ちます。
...
次に、Microsoft Foundry でプロジェクトを作成し、エージェントを設定し、関数 API をツールとして登録して、エージェントから呼び出せるようにします。
...
1. 新しいプロジェクトを作成します。
   ...
   1. ダイアログが表示されたら、**Microsoft Foundry リソース**を選択します。
...
1. **[ツール]** セクションまで下スクロールします。 **[追加]** を選択し、**[すべてのツールを参照]** を選択します。
