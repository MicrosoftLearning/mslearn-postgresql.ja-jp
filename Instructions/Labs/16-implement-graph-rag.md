...
1. この演習では、2 つの方法のうち **1 つ**を使用して Azure OpenAI に対して認証を行います。 環境に適用する方を選択し、各手順で該当する指示のみに従います。

    - **API キー** — Azure portal からコピーしたキーを使用する (ほとんどの環境で機能する)。
    - **マネージド ID** — Microsoft Entra ID トークン ベースの認証を使用する (組織レベルで API キーが無効になっている場合に必要)。
    ...
    **マネージド ID** をお使いで、その設定をまだ行っていない場合は、ここで次のコマンドを実行して行います。 それ以外の場合、次のステップに進みます。
    ...
    > **注:** 再起動後、サーバーが復旧し、ロールの割り当てが反映されるまで 2、3 分待ちます。 後の手順で認可エラーが発生する場合は、数分待ってからもう一度お試しください。
...
1. [Azure portal](https://portal.azure.com/) で、ツールバーの **Cloud Shell** アイコンを選択して Cloud Shell を開きます。

1. 次のコマンドを実行して、`ContosoHelpDesk` データベースに接続します。`<server-name>` を PostgreSQL フレキシブル サーバーの名前に置き換えます (Azure portal の PostgreSQL リソースの **Overview** ページに記載されています)。
...
1. 次に、Azure OpenAI への `azure_ai` 拡張機能の接続を構成します。 Azure OpenAI リソースのエンドポイントが必要です (Azure portal の **[リソース管理]** の下の **[キーとエンドポイント]** ページにあります)。
    ...
    *ContosoHelpDesk* のプロンプトが表示されたら、選択した認証方法のコマンドを実行します。
    ...
    > **マネージド ID の使用:** エンドポイントのみを設定します。 `subscription_key` が構成されていない場合、拡張機能はサーバーのシステム割り当てマネージド ID を自動的に使用します。
...
1. 既存の行に対する埋め込みをバックフィルします。
    ...
    **psql** セッション (Cloud Shell) で次のコマンドを実行し、埋め込みを複数のバッチで計算します。 これにより、行が小さいグループで処理され、各バッチ間で一時停止されるため、"**429 要求が多すぎる**" エラーが回避されます。
    ...
    108 行がすべてバックフィルされたら、「`\q`」と入力して *psql* を終了します。
...
1. **[Value]** 列で、有効な拡張機能のリストに `AGE` を追加します。
1. **[保存]**、**[保存して再起動]** の順に選んで、変更を適用します。

サーバーが再起動するまで、しばらくかかります。 再起動が完了すると、Apache AGE 拡張機能を PostgreSQL データベースで使用できるようになります。
...
1. 次の SQL ステートメントを実行してノードを追加します。
    ...
    > **注:** 関数は `void` を返すので、出力には空白の行が表示されます。これは想定される動作です。 重要なのは、行数 (108 個のポリシー ノード、8 個の部署、8 個のカテゴリ、20 個のトピック) です。
...
> &#128161; `psql` で、SQL クエリを実行する前に、`\set question` コマンドを**それだけの行**で実行し、**Enter** キーを押します。 両方を一度に貼り付けると、共通テーブル式 (CTE) が期待どおりに実行されないことがあります。
